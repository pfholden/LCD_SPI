...
LCDWIKI_SPI::LCDWIKI_SPI()
{
    ...
    // Enable outputs
    PORT->Group[GPIO_GROUP].DIRSET.reg |= DEFAULT_LED | DEFAULT_CD | DEFAULT_CS | DEFAULT_RST;
//    PORT->Group[GPIO_GROUP].DIRSET.reg = DEFAULT_CD;
//    PORT->Group[GPIO_GROUP].DIRSET.reg = DEFAULT_CS;
//    PORT->Group[GPIO_GROUP].DIRSET.reg = DEFAULT_RST;
	...
	yoffset = 0;
	rotation = 0;
 	lcd_model = current_lcd_info[DEFAULT_MODEL].lcd_id;
    //lcd_model = 0xFFFF;
	width = current_lcd_info[DEFAULT_MODEL].lcd_wid;
	height = current_lcd_info[DEFAULT_MODEL].lcd_heg;
    WIDTH = 240;
	HEIGHT = 320;
	setWriteDir();
}
...
{
  	CS_ACTIVE;
	if(lcd_driver == ID_1106 || lcd_driver == ID_9341)
	{
    	writeCmd8(cmd);
	}
	else
